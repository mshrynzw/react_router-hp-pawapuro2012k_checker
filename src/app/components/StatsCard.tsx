...
export function StatsCard({ title, stats, onChange, accentColor }: StatsCardProps) {
  ...
  return (
    ...
    >
      {/* グロー効果 */}
      <div className={`absolute -inset-0.5 bg-gradient-to-r ${accentColor} rounded-2xl blur opacity-30 group-hover:opacity-50 transition duration-500`} />
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
            ...
            <div className="grid gap-3">
              {baseStats.map(stat => (
                <div key={stat.id} className="flex items-center gap-4 bg-slate-800/50 rounded-xl p-4 border border-slate-700/30 hover:border-cyan-500/50 transition-colors">
          ...
          <Accordion.Root type="multiple" className="space-y-2">
            {categories.filter(cat => cat.id !== 'base').map(category => {
              const pitchesInCategory = pitchTypes.filter(p => p.category === category.name);

              return (
                <Accordion.Item key={category.id} value={category.id} className="border border-slate-700/30 rounded-xl overflow-hidden bg-slate-800/30">
                  <Accordion.Header>
                    <Accordion.Trigger className="w-full flex items-center justify-between p-4 hover:bg-slate-700/30 transition-colors group/trigger">
                      <span className="flex items-center gap-2 text-slate-200 font-medium">
                        ...
                        {category.name}
                        <span className="text-xs text-slate-400">({pitchesInCategory.length})</span>
                      </span>
                  ...
                  <Accordion.Content className="data-[state=open]:animate-accordion-down data-[state=closed]:animate-accordion-up overflow-hidden">
                    <div className="px-4 pb-4 space-y-2">
                      {pitchesInCategory.map(pitch => (
                        <div key={pitch.id} className="flex items-center gap-3 bg-slate-900/50 rounded-lg p-3 border border-slate-700/20 hover:border-cyan-500/30 transition-colors">
